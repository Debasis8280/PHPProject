...
<div class="grid_3">
  ...
  <div class="col-md-9 profile_left1">
      ...
      <?php
       ...
       $sql="select u.name,u.profileid,u.MATRIMONYPROFILEFOR,u.gender,u.profile,u.religion,u.COUNTRYLIVINGIN,p.age,p.height,p.MARITAL_STATUS,pr.HIGHEST_EDUCATION from users u natural join  personal_details p  natural join professional_details pr where u.COUNTRYLIVINGIN like'$location%' limit $starting_limit,$limit";
           ...
           if($pst->rowCount()>0)
           {
               while($row=$pst->fetch())
               {
                   if($row["gender"]!=$_SESSION["gender"]){
                       $profile=str_replace("../","",$row["profile"]);
                           echo "<div class='profile_top'><a href='userprofile?userid=$row[profileid]'>
                               <h2>$row[profileid]</h2>
                               <div class='col-sm-3 profile_left-top'>
                                   <img class='i'src='$profile' class='img-responsive' alt=''/>
                               </div>
                               <div class='col-sm-6'>
